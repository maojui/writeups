...
def get_conjugate(lhs, rhs):
	d = {}
	for c1, c2 in zip(lhs, rhs):
		for e1, e2 in zip(c1, c2):
			d[e1] = e2
	k = Permutation([d[i] for i in range(N)])
	return k
...
a_cycle = sorted(a1.cycles(), key=len)
b_cycle = sorted(b1.cycles()[::-1], key=len)

solve(a_cycle, b_cycle, a2, b2)