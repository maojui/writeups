...
secret = '48667ec1a5fb3383'

def s2b(s):
    return list(map(int, format(s2n(s), '0{}b'.format(8*len(s)))))
...
class zer0SPN(object):
    ...
    def get_roundkey(self, k):
        ...
        tmp = self.roundkey[self.key_size*k:self.key_size*(k+1)]
        print(k,[int(i) for i in tmp])
...
if __name__ == '__main__':
    ...
    print(b'Your flag is flag{secret}')
    f = open('test_data', 'wb')
    for _ in range(65536):
        c = zer0SPN(bytes.fromhex(secret))
